import pytest
from testdata.access import INSERT_ACCESS_DB
...
async def insert_superuser_refresh_token(prepare_users, get_postgres_session):
    ...
    await get_postgres_session.execute(INSERT_SUPERUSER_DEVICE_REQUEST)
...
@pytest.fixture(scope="function")
async def prepare_access(empty_db_tables, get_postgres_session):
    """Inserts roles, users, access to appropriate tables."""
    for user in USER_CREATION:
        await get_postgres_session.execute(user)
    for role in INSERT_ROLE_DB:
        await get_postgres_session.execute(role)
    for access in INSERT_ACCESS_DB:
        await get_postgres_session.execute(access)

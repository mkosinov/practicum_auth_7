...
async def test_success_registration(empty_db_tables, get_http_session, user):
    ...
    response = await get_http_session.post(
        url=ENDPOINT, headers=HEADERS, json=user
    )
...
async def test_wrong_login_registration(
    ...
    response = await get_http_session.post(
        url=ENDPOINT, headers=HEADERS, json=INVALID_USER_NAME
    )
    status = response.status
...
async def test_wrong_surname_registration(empty_db_tables, get_http_session):
    """Checks invalid registration with incorrect surname."""
    response = await get_http_session.post(
        url=ENDPOINT, headers=HEADERS, json=INVALID_USER_SURNAME
    )
    status = response.status
